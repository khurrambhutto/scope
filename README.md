...
- 📦 **Unified Package List** — See everything installed from APT, Snap, Flatpak, and AppImage in a single view, with icons and metadata resolved from `.desktop` entries.
...
2. **Snap** (`snap.rs`) — parses `snap list` and skips runtime/base snaps (`core*`, `gtk-*`, `gnome-*`, `snapd`, `bare`). On-disk size is measured from `/snap/<name>/current`.
3. **Flatpak** (`flatpak.rs`) — runs `flatpak list --app` with explicit tab-delimited columns. Only applications are reported; runtimes are excluded. The application id matches a `.desktop` id exactly.
...
1. **Preview** — the frontend calls `preview_uninstall(packageKey)`. The backend looks the package up in the cached scan, runs the **safety deny-list** (`safety/mod.rs`), detects whether a Flatpak is user- or system-installed, and builds an `OperationPlan` (package id, version, the exact command that will run, whether a password is needed). The plan is stored server-side in a `PlanStore` with a 5-minute TTL. The frontend receives it only for display.
...
| Flatpak (system) | `pkexec flatpak uninstall -y <id>` | Polkit popup |

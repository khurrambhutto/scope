...
impl SnapScanner {
    ...
    fn detect_app_type(name: &str) -> AppType {
        ...
        let desktop_paths = [
            format!("/var/lib/snapd/desktop/applications/{}_*.desktop", name),
        ];
        ...
        // Common GUI snaps
        let gui_snaps = [
            "firefox", "chromium", "vlc", "spotify", "slack", "discord",
            "code", "sublime-text", "gimp", "inkscape", "blender",
        ];
        ...
        Box::pin(async {
            ...
            for line in stdout.lines().skip(1) {
                ...
                if parts.len() >= 4 {
                    ...
                    if let Ok(info_output) = Command::new("snap")
                        .args(["info", &name])
                        .output()
                        .await
                    {
                        ...
                        for line in info.lines() {
                            if line.starts_with("summary:") {
                                package.description = line.trim_start_matches("summary:").trim().to_string();
                                break;
                            }
                        }
                    }
